...
<div class="flex h-screen overflow-hidden bg-slate-100">
    ...
    <aside class="fixed inset-y-0 left-0 z-50 flex flex-col transition-all duration-300 ease-in-out bg-slate-900 border-r border-slate-800"
        ...
        <div class="flex items-center gap-3 px-4 h-16 border-b border-slate-800 shrink-0">
            ...
            @else
                <div class="w-9 h-9 rounded-xl bg-amber-500 flex items-center justify-center text-slate-950 font-black text-xs shrink-0">
                    GMF
                </div>
            @endif
        ...
        <div class="border-t border-slate-800 p-3 shrink-0 bg-slate-950/40">
            <div class="flex items-center gap-3" x-show="sidebarOpen" x-transition.opacity>
                <div class="w-9 h-9 rounded-xl bg-amber-950 border border-amber-800 text-amber-400 flex items-center justify-center font-bold text-sm shrink-0">
                    {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                </div>
                <div class="min-w-0 flex-1">
                    <p class="text-slate-200 text-xs font-semibold truncate">{{ auth()->user()->name }}</p>
                    <p class="text-amber-400/80 text-[11px] truncate">Owner / Pemilik</p>
                </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden transition-all duration-300" :class="sidebarOpen ? 'lg:ml-64' : 'lg:ml-20'">
        ...
        <header class="h-16 bg-white border-b border-slate-200/80 flex items-center justify-between px-4 sm:px-6 shrink-0 shadow-sm z-30">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-base font-bold text-slate-900 font-display">@yield('page-title', 'Dashboard Owner')</h1>
                    <p class="text-xs text-slate-500 font-medium">@yield('page-subtitle', 'Executive Overview Gintung Master Fitness')</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-xl bg-slate-900 text-amber-400 flex items-center justify-center font-bold text-xs">
                    {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                </div>
            </div>
        </header>
